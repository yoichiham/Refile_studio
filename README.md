...
**公開URL**: https://yoichiham.github.io/Refile_studio/

## 特徴

- **プライバシー優先** — ファイルやテキストはすべて端末内で処理。サーバーへの送信なし。
- **インストール不要** — ブラウザだけで動く。
- **PWA 対応** — 初回ロード後はオフラインでも利用可能。Markdown→PDF の日本語フォント（約8.8MB）は、オンライン中に Markdown ツールを開くとバックグラウンドで自動取得（Save-Data/低速回線時は手動取得ボタンに切替）。
